  ...
  ({ label, error, className = '', ...props }, ref) => (
    <div className="w-full">
      ...
      <div className="relative">
        <input
          ref={ref}
          className={`peer w-full py-2.5 bg-transparent text-[14px] text-neutral-900 dark:text-slate-100 placeholder-neutral-300 dark:placeholder-slate-600 focus:outline-none ${className}`}
          {...props}
        />
        {/* static base line */}
        <div className={`absolute bottom-0 left-0 right-0 h-px ${error ? 'bg-red-200' : 'bg-neutral-200 dark:bg-slate-700'}`} />
